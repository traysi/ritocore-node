...
/**
 * Will remove a service from ritocore-node.json
...
/**
 * Will remove the Node.js service and from the ritocore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The ritocore-node configuration directory
...
function remove(options, done) {
  ...
  var ritocoreConfigPath = path.resolve(configPath, 'ritocore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(ritocoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a ritocore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to ritocore-node.json
        removeConfig(ritocoreConfigPath, service, next);
      });
    }, done
  );
}
