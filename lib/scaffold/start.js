...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for ritod is necessary for this upgrade with the "reindex=1" rito.conf option. \n' +
                 'There are changes necessary in both rito.conf and ritocore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/traysi/ritocore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed ritocore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './ritocore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in ritocore-node services directory
    ...
    if (servicePackage.ritocoreNode) {
      serviceModule = service.name + '/' + servicePackage.ritocoreNode;
    }
 ...
 * @param {Node} node - The Ritocore Node instance
 */
