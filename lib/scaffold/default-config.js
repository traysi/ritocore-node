...
/**
 * Will return the path and default ritocore-node configuration. It will search for the
 * configuration file in the "~/.ritocore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.ritocore');
  var defaultConfigFile = path.resolve(defaultPath, './ritocore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['ritod', 'web'];
